...
https://drive.google.com/file/d/1GGKzyfYM_iYrEQhiNUWosPciuGtGkBnz/view?usp=sharing
...
Tests
```{r}
m <- lmer(IA_DWELL_TIME ~ 1 + (1|SUBJECT) + (1|WORD_ID), data=filter(celer_ia,shared_text == 1,English =="L1"))
summary(m)
```
...
get_fixed_effects <- function(report, parametrization){
      ...
      if(parametrization == "nested"){
          m <- lmer(RT ~ English/FREQ + English/SURP_GPT2 + English/WORD_LEN + (1| SUBJECT), data=report)
      }else{
...
coefs_geco <- geco_oov_long %>% group_by(DATASET, fix_measure) %>% do(get_fixed_effects(., "nested"))
...
```
Interactions of word propety effect and native language
```{r}
inter_geco <- geco_oov_long %>% group_by(DATASET, fix_measure) %>% do(get_fixed_effects(., "unnested"))
inter_celer <- celer_ia_oov_long %>% group_by(DATASET, fix_measure) %>% do(get_fixed_effects(., "unnested"))
```
